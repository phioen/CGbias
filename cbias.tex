...
we use $w_{0,800}=0.102$ arcsec, which is the same as in ES13.
...
%
\begin{figure}
\centerline{\includegraphics[width=6.0cm]{../plot/zairy.eps}}
...
Airy function in GalSim with diameter $D=2.5$ and
obscuration $0.33$. As shown in ES13, one needs to correct the PSF
effect in the observed images, or the CG bias will be
underestimated. We perform deconvolution for images before the
bias estimation. It is a straightforward mathematical step for the
...
the two images will be fitted with different initial parameters. %($>100\%$),
...
\begin{figure}
  ...
  %\includegraphics[width=8.0cm]{zfitsB.eps}
  %\includegraphics[width=8.0cm]{zfitsS.eps}
...
PSF model of Euclid survey, we use an Airy function,
which is close to the Design profile \citep{2011arXiv1110.3193L}
%
\be
...
\eea
%
The integral of $P(r)$ in the first term is independent of the image of the galaxy, and equals to unity for normalised PSF. In case of circular symmetric PSF or the asymmetry is small, the second term can be negelect. $Q_{ij}'\approx p Q_{ij}$, where $p$ is only determined by the size of PSF. Thus, for small PSF we have the approximation
%
\be
...
\ee
%
which means that the small circular PSF will not change the CG bias in our definition. In general case, the relation depends on the properties of the PSF and the size of weight function. It requires detail study from image simulations.
