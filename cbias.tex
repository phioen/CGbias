...
\newauthor{M. Vicinanza$^{1,4,5}$, Bryan Gillis$^{6}$ L. Miller$^{7}$, J. Rhodes$^{8,9}$}
\\
...
Moreover, it has been shown that using two high resolution narrow band
images, one can calibrate the CG bias with high precision for a
...
result and the method in \citet{2013MNRAS.432.2385S}(ES13 in the rest
of this paper), and apply the calibration method to the simulated
noisy images. The method requires that the match of the bandwidth and
the smooth spectrum of the source galaxy. For the first point, the
bandwidth of weak lensing survey on Euclid can be cover by two filters
on the Hubble Space Telescope (HST), i.e., F606W and F814W. In the
image simulation with smooth spectra, the calibration can reproduce
the CG bias with high precision.
...
colour gradient of the galaxy are correlated
\citep{2010MNRAS.407..144T} and (Welikala et al. in prep.), thus it is
not a surprise to see that the CG bias has a correlation with the
colour of source galaxy. In Section 3, we follow the result in
ES13. We use simulated images to study the CG bias. In Section 4 we
apply the calibration method using two band images from CANDELS data,
and give our discussion and conclusion in the end.
